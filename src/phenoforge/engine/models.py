...
DESIGN.md's tiered-provenance commitment (see "Tiered provenance") means a
concept set is never a bare list of codes: every concept carries where it
came from, and terms that could not be resolved are reported rather than
silently dropped.
"""
...
class ProvenanceTier(StrEnum):
    ...
    :cvar CURATED: From the OHDSI Phenotype Library. Use as-is (DESIGN.md D4).
        Not yet produced by anything in this codebase.
    :cvar PUBLISHED: From a validated published algorithm. Deferred to v2
        per DESIGN.md.
...
class Concept(BaseModel):
    ...
    :ivar vocabulary_id: Source vocabulary. Only ``"ICD10CM"`` is ever
        surfaced outside the engine (DESIGN.md D9).
    """
